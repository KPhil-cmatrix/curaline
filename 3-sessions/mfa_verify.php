...
$stmt = $conn->prepare("
    SELECT staff_id, first_name, last_name, staff_role, mfa_secret
    FROM staff_info
    WHERE staff_id = ?
    LIMIT 1
");
    ...
    if ($code === '') {
        ...
        try {
            ...
            if ($isValid) {
                ...
                unset($_SESSION['pending_mfa_last_name']);
                unset($_SESSION['pending_mfa_secret']);

                if ($user['staff_role'] === 'Admin') {
                    header("Location: ../6-admin/admin.php");
                } else {
...
<head>
  ...
  <title>Curaline – Verify MFA</title>
...
<body class="min-h-screen bg-gradient-to-br from-[#EEF3FA] to-[#C9D8F0] flex items-center justify-center px-4">
  ...
  <div class="app-card p-8 w-full max-w-md">
    ...
    <p class="text-xs text-gray-400 mt-4 text-center">
      Signing in as <?= htmlspecialchars($user['staff_id']) ?>
    </p>
  </div>
