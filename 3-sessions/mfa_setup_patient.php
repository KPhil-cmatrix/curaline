...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($code === '') {
        ...
        if ($isValid) {
            $stmt = $conn->prepare("UPDATE patient_info SET mfa_secret = ?, mfa_enabled = 1 WHERE patient_id = ?");
            $stmt->bind_param("ss", $secret, $user_id);
            $stmt->execute();

            unset($_SESSION['pending_mfa_secret']);

            header("Location: mfa_verify_patient.php");
            exit;
        } else {
...
<head>
  ...
  <title>Curaline – Patient MFA Setup</title>
